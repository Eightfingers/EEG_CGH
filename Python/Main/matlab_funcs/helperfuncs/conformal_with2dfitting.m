...
addpath('C:\Users\Souganttika\OneDrive\Documents\MATLAB\Data\7_2_2022-20220208T043500Z-001\7_2_2022');
...
% noise_y = rand(size(circumference_x)) * noiseIntensity;
% noise_z = rand(size(circumference_x)) * noiseIntensity;
%%% Circumference
circumference_dataset= new_markers_circum;
circumference_dataset = rmmissing(circumference_dataset);
% circumference_dataset = circumference_dataset*R;
...
% noise_x = rand(size(e2e_x)) * noiseIntensity;
% noise_y = rand(size(e2e_x)) * noiseIntensity;
% noise_z = rand(size(e2e_x)) * noiseIntensity;
%%% Ear to Ear
e2e_dataset= new_markers_e2e;
e2e_dataset = rmmissing(e2e_dataset);
% e2e_dataset = e2e_dataset*R;
% noiseIntensity = 0.005; %
...
%% Find Centre
% [latcentre, loncentre, hcentre, ~] = geocent_inv(0,0,0, ellipsoid); % Centre considered at (0,0,0)
...
%%% Circumference
% [x_new_circum, y_new_circum, azi, rk] = eqdazim_fwd(latcentre,loncentre, latpoints_circum, lonpoints_circum, ellipsoid);
...
%%% Ear to Ear
% [x_new_e2e, y_new_e2e, azi, rk] = eqdazim_fwd(latcentre,loncentre, latpoints_e2e, lonpoints_e2e, ellipsoid);
[x_new_top_e2e, y_new_top_e2e, azi, rk] = eqdazim_fwd(latcentre_top,loncentre_top, latpoints_e2e, lonpoints_e2e, ellipsoid);


%%% Nasion to Inion
% [x_new_nziz, y_new_nziz, azi, rk] = eqdazim_fwd(latcentre,loncentre, latpoints_nziz, lonpoints_nziz, ellipsoid);
...
hold on
scatter(x_new_top_e2e,y_new_top_e2e,'r*');
hold on
scatter(x_new_top_nziz,y_new_top_nziz,'go');
...
[row_startpoint_e2e,~] = find(trans_newmat_xy==closest_startpoint_e2e);
[row_endpoint_e2e,~] = find(trans_newmat_xy==closest_endpoint_e2e);

new_matrix_x_e2e = (trans_newmat_xy(row_endpoint_e2e:row_startpoint_e2e,1)).';
new_matrix_y_e2e = (trans_newmat_xy(row_endpoint_e2e:row_startpoint_e2e,2)).';
...
new_matrix_x_nziz = (trans_newmat_xy_nziz(row_startpoint_nziz:row_endpoint_nziz,1)).';
new_matrix_y_nziz = (trans_newmat_xy_nziz(row_startpoint_nziz:row_endpoint_nziz,2)).';
...
%% Compare to Static
static = daniellestatic;
static_new = [];
j = 1;
k = j +2 ;
while k <= length(static)
    static_new(end+1, :) = [static(j:k)];
    j = j + 3;
    k = j + 2;
end 
