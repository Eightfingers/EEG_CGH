...
circumference = readmatrix('CIRCUM_13_1_2013.csv');
ear2ear = readmatrix('EAR2EAR_13_1_2014.csv');
nziz = readmatrix ('NZIZ_13_1_2012.csv');
...
nziz_specs_quaternion_extracted = specs_data(:,1:4);
nziz_specs_quaternion_extracted = rmmissing(nziz_specs_quaternion_extracted);
...
nziz_specs_quaternion_extracted = [nziz_specs_quaternion_extracted(:,4), nziz_specs_quaternion_extracted(:,1), nziz_specs_quaternion_extracted(:,2), nziz_specs_quaternion_extracted(:,3)];
...
new_markers_nziz = [];
new_markers_nziz = transform_frame_quat(stylus_dis_matrix, nziz_specs_quaternion_extracted, specs_dis_matrix);
...
final_quat = nziz_specs_quaternion_extracted(end:end, :);
rep_final_quat = repmat(final_quat, length(predicted), 1);
% new_markers_nziz = transform_frame_quat(stylus_dis_matrixw, nziz_specs_quaternion_extracted, specs_dis_matrix);

% negative displacement!!
final_global_markers = transform_frame_global(predicted, rep_final_quat, -specs_dis_matrix);

hold on;
plot3(final_global_markers(:,1), final_global_markers(:,2), final_global_markers(:,3), 'o');
