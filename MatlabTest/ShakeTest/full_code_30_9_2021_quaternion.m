...
%% Load the different wanded data
%%% Circumference
circumference = readmatrix('quat_circum_shake_30_9_2023.csv'); 
circum_wand = circumference(:,3:9); 
circum_wand = rmmissing(circum_wand);
circum_wand = [circum_wand(:,5), circum_wand(:,7),circum_wand(:,6)];

plot3(circum_wand(:,1), circum_wand(:,2), circum_wand(:,3), 'd');
hold on;

circum_specs= circumference(:,35:41); 
circum_specs = rmmissing(circum_specs);

quaternion_extracted = circum_specs(:,1:4); % extract the rotation vector out
dis_matrix_circum = circum_specs(:,5:7); % extract the displacement vector out
...
new_markers_circum = transform_frame_quat(circum_wand, quaternion_extracted, dis_matrix_circum);
...
plot3(e2e_wand(:,1),e2e_wand(:,2),e2e_wand(:,3),'d');
hold on;

e2e_specs= ear2ear(:,35:41); 
e2e_specs = rmmissing(e2e_specs);
quaternion_extracted = e2e_specs(:,1:4); % extract the rotation vector out
quaternion_extracted = [quaternion_extracted(:,4), quaternion_extracted(:,1), quaternion_extracted(:,3), quaternion_extracted(:,2)];
dis_matrix_e2e = e2e_specs(:,5:7); % extract the displacement vector out
dis_matrix_e2e = [dis_matrix_e2e(:,1), dis_matrix_e2e(:,3), dis_matrix_e2e(:,2)];

%% Transform to specs frame
new_markers_e2e = transform_frame_quat(e2e_wand, quaternion_extracted, dis_matrix_e2e);

plot3(new_markers_e2e(:,1),new_markers_e2e(:,2),new_markers_e2e(:,3),'d');
hold on;

%%% NZIZ
nziz = readmatrix ('quat_NZIZ_shake_30_9_2021.csv');
nziz_wand = nziz(:,3:9); 
nziz_wand = [nziz_wand(:,5),nziz_wand(:,7),nziz_wand(:,6)];
...
quaternion_extracted = nziz_specs(:,1:4); % extract the rotation vector out
...
quaternion_extracted = [quaternion_extracted(:,4), quaternion_extracted(:,1), quaternion_extracted(:,3), quaternion_extracted(:,2)];
new_markers_nziz = transform_frame_quat(nziz_wand, quaternion_extracted, dis_matrix_nziz);

plot3(new_markers_nziz(:,1),new_markers_nziz(:,2),new_markers_nziz(:,3),'d');
hold on;

% %%% Static
% static = readmatrix('Static_30_9_2021');
% static_specs = static(8,3:8); % specs pose
...
title('predicted 21 eeg positions');
% scatter3(static_dataset(:,1), static_dataset(:,2),static_dataset(:,3));
% for i = 1:1:length(static_dataset)
%     text(static_dataset(i,1), static_dataset(i,2), static_dataset(i,3),string(i));
% end
